...
export function useMembers(scope: MemberApiScope) {
  ...
  return {
    ...
    isLoading: members.isLoading || roles.isLoading || permissions.isLoading,
    isSaving: status.isPending || role.isPending || createRole.isPending || updateRole.isPending || deleteRole.isPending,
    error: members.error || roles.error || status.error || role.error,
  };
}
