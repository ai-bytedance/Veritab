...
export default function OrganizationResourceDrawer({ organization, kind, onClose }: { organization: OrganizationSummary; kind: DrawerKind; onClose: () => void }) {
  ...
  const addMember = async (user: RegisteredUser) => {
    setSavingId(user.id);
    try { await apiRequest(`/organizations/${organization.id}/members`, { method: "POST", body: JSON.stringify({ userId: user.id, roleCode: "org_admin" }) }); setMessage(`${user.displayName} 已加入组织`); await load(); }
  ...
  return <div className="fixed inset-0 z-[130] bg-slate-900/30" onMouseDown={onClose}><aside className="ml-auto flex h-full w-full max-w-4xl flex-col bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="flex-1 overflow-auto px-6 pb-6">
      ...
      : kind === "members" ? <table className="w-full text-left text-xs"><thead className="sticky top-0 bg-slate-50 text-slate-500"><tr><th className="px-4 py-3">姓名 / 账号</th><th>角色</th><th>邮箱</th><th>状态</th></tr></thead><tbody className="divide-y divide-slate-100">{visibleMembers.map(({ user, status }) => <tr key={user.id}><td className="px-4 py-4 font-bold">{user.displayName}<div className="font-mono text-[10px] font-normal text-slate-400">@{user.username}</div></td><td>{user.roleBindings.filter((item) => item.scopeType === "ORGANIZATION").map((item) => item.role.code === "org_admin" ? "系统管理员" : item.role.name).join("、") || "—"}</td><td className="text-slate-500">{user.email || "—"}</td><td className="text-emerald-600">{status === "ACTIVE" ? "启用" : "停用"}</td></tr>)}</tbody></table>
