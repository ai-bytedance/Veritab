...
import OrganizationRoleManagement from "./tenant/OrganizationRoleManagement";
...
export default function OrganizationSpaceSettings(props: Props) {
  ...
  const tabs: Array<[Tab, string]> = [["organizations", "组织管理列表"], ["projects", "项目空间"], ["roles", "角色权限"], ["members", "成员管理"]];
  ...
  return <div className="space-y-4">
    ...
    {(tab === "organizations" || tab === "projects") && <div className="flex flex-col gap-3 rounded-2xl bg-white px-5 py-4 shadow-sm md:flex-row md:items-center md:justify-between"><div><h2 className="text-sm font-black">{tab === "organizations" ? "组织管理列表" : `${organization.name} · 项目空间`}</h2><p className="mt-1 text-[11px] text-slate-400">{tab === "organizations" ? "组织是成员、角色和项目空间的隔离边界" : "项目空间 ID 与 Key 均由系统自动生成"}</p></div><div className="flex gap-2"><label className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400"/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索名称或 ID" className="rounded-xl bg-slate-50 py-2 pl-9 pr-3 text-xs outline-none"/></label><button onClick={() => openEditor(tab === "organizations" ? "create-organization" : "create-project")} className="inline-flex items-center gap-1 rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-white"><Plus className="h-4 w-4"/>新增{tab === "organizations" ? "组织" : "项目"}</button></div></div>}
    ...
    {tab === "roles" && <section className="overflow-hidden rounded-2xl bg-white shadow-sm"><OrganizationRoleManagement scope={{ organizationId: organization.id }} onAddUser={() => setDrawer({ organization, kind: "add-member" })}/><div className="border-t border-slate-100"><ProjectRoleManagement organizationId={organization.id} project={project}/></div></section>}
